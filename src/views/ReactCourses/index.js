...
function ReactCourses() {
  ...
  const reactCourses = useSelector(selectAllCourses);
  const courseStatus = useSelector(selectCourseStatus);
  const currentCourseType = useSelector(selectCourseType);
  const courseType = 'react';

  useEffect(() => {
    document.title = 'React Courses | UI Courses';
  ...
  if (courseStatus === statusType.loading) {
    ...
    return (
      <div className='reactCourses'>
        <ul>
          {reactCourses.map((course, index) => (
            <li key={index}>
    ...
    return <div>Fetch React course data failed</div>;
  } else {
    return <div>React courses Page Error</div>;
  }
}
